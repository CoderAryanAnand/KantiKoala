...
function updateStarButtons(isStarred) {
    ...
    btns.forEach(btn => {
        if (isStarred) {
            ...
            // Important: Re-add perspective/transform fixes when updating class
            if (btn.id === 'star-btn-front' || btn.id === 'star-btn-back') {
               // The styles are inline, so they persist, but class needs to be correct.
               // We removed backface-hidden from class and rely on parent container + inline translateZ
            }
        ...
        // Ensure translateZ is kept/re-applied if damaged by class update (though style shouldn't be affected)
        btn.style.transform = 'translateZ(1px)';
        btn.style.webkitTransform = 'translateZ(1px)';
    });
}
